...
class GenerationMixin:
    ...
    # def relative_top_filter(self, scores: torch.FloatTensor, relative_top: float = 0.1, filter_value: float = -1000.0, min_tokens_to_keep: int = 1) -> torch.FloatTensor:
    #     scores_normalized = scores.log_softmax(dim=-1) 
    #     sorted_logits, sorted_indices = torch.sort(scores_normalized, descending=True)
    #     min_thresh = sorted_logits[..., min_tokens_to_keep-1] 
    #     probs_max = torch.max(scores_normalized, dim=-1).values
...
###加入safe_logits
        ...
        while True:
            ...
            if base_layer is not None:
                ...
                if relative_top > 0.0:
                    ...
                    # if(toxic_layer<=13):
                    #     final_logits = self.relative_top_filter_safe(final_logits, relative_top)
                    # final_logits = self.relative_top_filter(final_logits, relative_top)
                    safe_logits = self.relative_top_filter(safe_logits, relative_top)

                    
                    mask = safe_logits[0] < -1e3
                    base_logits[0][mask] = -1e3
                print("---------------------------dola_greedy_decode在筛选后：-------------------------------")
                print("final_logits:{}", final_logits)
                print("base_logits:{}", base_logits)
                print("safe_logits:{}", safe_logits)    
                ...
                #     logits = final_logits - base_logits
                # else:
                #     logits = safe_logits - toxic_logits

                # logits = final_logits - base_logits-toxic_logits

                logits = safe_logits - base_logits
    ...
    def dola_sample(
       ...
       # 如果启用了 n-gram 避免机制
            ...
            # if eos_token was found in one sentence, set sentence to finished
            if eos_token_id_tensor is not None:
                # 生成的 token 如果是 eos_token_id，将其标记为完成
                eos_in_next_tokens = next_tokens == eos_token_id_tensor  # 检查生成的 token 是否是 eos_token_id
                unfinished_sequences = unfinished_sequences * (~eos_in_next_tokens).long()  # 未完成的序列标记
                if unfinished_sequences.max() == 0:
                    break  # 如果所有序列都已经生成了 eos_token_id，停止生成
